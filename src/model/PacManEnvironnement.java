...
public class PacManEnvironnement extends Environnement {
	...
	public PacManEnvironnement(SMA sma, int taille, boolean torique, ArrayList<Agent> chasseurs) {
		...
		// On place le joueur :
		/**ici sa plante parce que la liste cellule n'est pas encore initilisé et tu essaie de chercher des cellules vide, alors qu'elle sont tout a null, le .isempty() donne nullpointer**/
		//int x =getRandomCoord(-1);
		//int y =getRandomCoord(x);
		/**better to be in SMA ici le la liste n'est pas encore initialisé par l'environement, le .clear du randomPut supprime l'avatar**/
		//avatar = new Avatar(x,y,this);
		//this.agents.add(avatar);
		/**null pointer la cellule que tu veux setter et encore a null**/
		//this.espace[x][y].setAgent(avatar);

		this.finish = false;

		// Init tableau des distances //better to be in AGENTChasseur
